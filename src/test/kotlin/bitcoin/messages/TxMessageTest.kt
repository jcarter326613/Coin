...
class TxMessageTest {
    ...
    fun inAndOut_Normal_Success() {
        val message = createRandomTx()
        val messageByteArray = message.toByteArray()
        val outMessage = TxMessage.fromByteArray(messageByteArray).value

        assertTxEqual(message, outMessage)
    }

    companion object {
        fun assertTxEqual(message: TxMessage, outMessage: TxMessage) {
